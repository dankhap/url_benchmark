...
sbatch --export=ALL,A="fine group_name=${algo}_${env}_run_finetune pretrained_agent=${algo} task=${env}_run snapshot_ts=2000000 seed=${seed} batch_sched=normal obs_type=${obs} " ./urlb_job_git.sh
sbatch --export=ALL,A="fine group_name=${algo}_${env}_run_buffered pretrained_agent=${algo} task=${env}_run snapshot_ts=0 seed=${seed} batch_sched=normal obs_type=${obs} buffer_dir=/code/url_benchmark/${algo}_${env}_${obs}_buffer${seed}" ./urlb_job_git.sh
sbatch --export=ALL,A="fine group_name=${algo}_${env}_run_buffered_partial pretrained_agent=${algo} task=${env}_run snapshot_ts=2000000 batch_sched=normal seed=${seed} obs_type=${obs} buffer_dir=/code/url_benchmark/${algo}_${env}_${obs}_buffer${seed} load_only_encoder=true" ./urlb_job_git.sh
